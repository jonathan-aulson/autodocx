...
def _read_plan_yaml(plan_path: Path) -> Tuple[Dict, str]:
    """
    Returns (header_dict, body_str). If no front matter present, returns ({}, full_text).
    """
    ...
    if raw.lstrip().startswith("---"):
        parts = raw.split("\n", 2)
        # parts[0] is '---', need to find the next '---'
        # safer: split on top-level '---' blocks
        blocks = raw.split("\n---\n", 1)
        if raw.startswith("---\n") and len(blocks) == 2:
            header_txt = blocks[0][4:]  # remove leading '---\n'
            body = blocks[1]
        else:
            # fallback: try second delimiter line-by-line
            fm = []
            lines = raw.splitlines(True)
            idx = 1
            while idx < len(lines) and not lines[idx].startswith("---"):
                fm.append(lines[idx]); idx += 1
...
def _write_plan_yaml(plan_path: Path, header: Dict, body: str) -> None:
    header_txt = yaml.safe_dump(header, sort_keys=False).strip()
    text = f"---\n{header_txt}\n---\n\n{body}"
...
def name_family(process_name: str) -> Optional[str]:
    for pat in NAME_FAMILY_PATTERNS:
        ...
        if m and m.group("fam"): return m.group("fam").lower()
    return None
...
def build_interdependency_graph(all_sirs: List[Dict], packaging: Dict) -> Dict:
    ...
    # Convert sets -> json lists
    node_index_json = {}
    for name,node in node_index.items():
        node_index_json[name] = {
            "family": node["family"],
            "identifiers": sorted(list(node["identifiers"])) if isinstance(node.get("identifiers"), set) else (node.get("identifiers") or []),
            "jdbc_targets": sorted(list(node["jdbc_targets"])) if isinstance(node.get("jdbc_targets"), set) else (node.get("jdbc_targets") or []),
...
# ==== PATCH: LLM repo-wide planning & follow-up =================================

def _collect_doc_corpus(docs_dir: Path, char_budget: int = 120_000) -> Tuple[str, List[str]]:
    """
    Read the generated Markdown docs and build a bounded corpus string for the planner LLM.
    - Prefer headings + first ~3KB per doc to keep the context compact.
    - Returns (corpus_text, doc_paths)
    """
    doc_paths = sorted([p for p in docs_dir.glob("*.md") if p.name != "_index.md"])
    chunks = []
    ...
    for p in doc_paths:
        ...
        # Pull front matter + headings and a small body sample
        lines = raw.splitlines()
        fm, body = [], []
        if lines and lines[0].strip() == "---":
            # capture YAML front matter
            i = 1
        ...
        headings = [ln for ln in body if ln.strip().startswith("#")]
        sample = "\n".join(body[:200])  # ~ small sample
        snippet = f"\n\n=== FILE: {p.name} ===\n" \
                  f"--- FRONTMATTER (keys only) ---\n" \
                  f"{', '.join([ln.split(':')[0] for ln in fm if ':' in ln][:24])}\n" \
                  f"--- HEADINGS ---\n" \
                  f"{chr(10).join(headings[:20])}\n" \
                  f"--- SAMPLE ---\n{sample}\n"
    ...
    corpus = (
        "You will receive a bundle of auto-generated docs produced from TIBCO BW sources.\n"
        "Each file begins with === FILE: <name> ===. Use this corpus to propose a business-first documentation plan.\n"
        + "".join(chunks)
    )
    return corpus, [p.name for p in doc_paths]


def _llm_make_repo_plan(corpus: str, visible_files: List[str], out_dir: Path) -> str:
    """
    Ask the LLM to write a strong, business-focused plan (YAML inside Markdown)
    describing the ideal doc set (audiences, filenames, outlines, acceptance criteria).
    Writes out/out/dox_draft_plan.md and returns its text.
    """
    try:
        import openai  # type: ignore
    except Exception:
        # Fallback: write a minimal stub plan
        plan_stub = (
            "# Draft Documentation Plan (stub)\n\n"
            "LLM unavailable. Create a Business Overview and a Movie Services Overview.\n"
            "```yaml\n"
            "docs:\n"
            "  - filename: business_overview.md\n"
            "    title: Business Overview of Integration Landscape\n"
            "    audience: business\n"
            "    goal: Explain capabilities and data flows in plain English\n"
            "    outline:\n"
            "      - What problems the system solves\n"
            "      - Key services (credit check, movie search) and how they interact\n"
            "      - Example user journeys\n"
            "    accept:\n"
            "      - Clear glossary\n"
            "      - 2 concrete end-to-end examples\n"
            "  - filename: movie_suite_overview.md\n"
            "    title: Movie Suite (Search/Get/Sort) Overview\n"
            "    audience: business\n"
            "    goal: Explain how movie processes work together\n"
            "    outline:\n"
            "      - What each process does (SearchMovies, GetRatings, SortMovies)\n"
            "      - Typical request/response examples\n"
            "      - Dependencies and failure modes (plain-English)\n"
            "    accept:\n"
            "      - One user story narrative\n"
            "```\n"
        )
        out_path = out_dir / "dox_draft_plan.md"
        atomic_write_text(out_path, plan_stub)
        return plan_stub

    prompt = (
        "You are a senior technical writer tasked with turning auto-generated BW process docs into "
        "business-readable documentation.\n"
        "Readers: business & product stakeholders with no BW background.\n"
        "Deliverables: a PRIORITIZED plan that will guide a separate tool to create new, holistic docs.\n\n"
        "Constraints & Output Requirements:\n"
        "1) Include a short rationale for the doc set.\n"
        "2) Include a YAML block under ```yaml named docs with fields:\n"
        "   - filename (lowercase, kebab-or-snake-case, .md),\n"
        "   - title,\n"
        "   - audience: business|ops|exec,\n"
        "   - goal: one sentence of value,\n"
        "   - outline: 5–10 bullet strings (plain English),\n"
        "   - depends_on: optional list of other filenames,\n"
        "   - accept: 3–6 acceptance bullets that a reviewer can check.\n"
        "3) Include at least one holistic, cross-process document (e.g., Movie suite, Credit check flow) that ties related processes together.\n"
        "4) Use the corpus. Make specific references to capabilities you see (e.g., credit score lookup, movie search → get ratings → sort).\n"
        "5) Keep prose specific; avoid generic boilerplate.\n"
        "6) Include a short 'Risks/Unknowns' section.\n"
        "7) Include a Related Documents section linking other relevant docs by name (links should be filename.md and use relative paths).\n"
        "8) Enhance the Markdown sections with MkDocs Material–compatible visuals/icons. Preserve all text exactly, only add icons/tables/emojis where appropriate.\n"
    )

    try:
        client = openai.OpenAI()
        model = os.environ.get("OPENAI_MODEL", "gpt-5-chat-latest")
        resp = client.chat.completions.create(
            model=model,
            temperature=0,
            max_tokens=1400,
            messages=[
                {"role": "system", "content": "Write exceptionally concrete, business-friendly documentation plans."},
                {"role": "user", "content": prompt},
                {"role": "user", "content": f"Visible files in .\\out\\docs: {', '.join(visible_files)}"},
        ...
        plan_text = (
            f"# Draft Documentation Plan (error fallback)\n\nLLM error: {e}\n\n"
            "Create: business_overview.md and movie_suite_overview.md (see earlier stub for structure).\n"
        )

    out_path = out_dir / "dox_draft_plan.md"
    atomic_write_text(out_path, plan_text)
    return plan_text
# ==== /PATCH ====================================================================
...
def _collect_doc_corpus(docs_out: Path) -> Tuple[str, List[str]]:
    """Read all .md docs under docs_out and return concatenated corpus + file list."""
    texts, names = [], []
...
def _llm_make_repo_plan(corpus: str, visible_files: List[str], out_dir: Path) -> str:
    """
    Ask LLM for a repo-wide documentation plan. Guarantee a YAML front-matter
    with 'docs:' so the follower can act, even if LLM returns weak content.
    """
    plan_path = out_dir / "dox_draft_plan.md"
    yaml_header = {
        "plan_version": 1,
        "docs": [
            # minimal safe fallback; LLM will typically expand/replace this
            {"title": "Business Overview of Movie Services",
             "filename": "BUSINESS_MOVIE_OVERVIEW.md",
             "inputs": visible_files[:12]},
            {"title": "Credit Services — Plain-English Guide",
             "filename": "BUSINESS_CREDIT_OVERVIEW.md",
             "inputs": visible_files[:12]}
        ]
    }
    header_text = "---\n" + yaml.safe_dump(yaml_header, sort_keys=False) + "---\n\n"

    body_intro = (
        "## Draft Documentation Plan\n"
        "The following plan translates technical BW artifacts into business-first documents.\n"
        "Each item in the YAML `docs:` header specifies a target file to be generated.\n"
        "Context corpus below is for reference.\n\n"
        "## Context Corpus (truncated)\n"
    )
    corpus_snippet = corpus[:120000]  # keep plan file manageable

    plan_text = header_text + body_intro + corpus_snippet

    # Try LLM enrichment (optional). If it fails, we still write the fallback header + corpus.
    try:
        import openai  # type: ignore
        cfg = load_llm_config()
        client = openai.OpenAI()
        prompt = (
            "You are a senior tech writer. Propose a detailed, business-oriented documentation plan for "
            "the system described in the corpus. Your output MUST preserve the YAML front matter and "
            "augment the `docs:` with 6–12 concrete deliverables (titles + filenames). "
            "Write specific, plain-English objectives for each deliverable after the YAML."
        )
        resp = client.chat.completions.create(
            model=cfg.get("model", "gpt-5-chat-latest"),
            temperature=0,
        ...
        maybe = resp.choices[0].message.content or ""
        # If the model forgot YAML, keep our header and append its body.
...
#=======================================
#---------------------------------------
# --- Front-matter helpers (paste above CLI) -----------------------------------
...
def run(env_file, root, sir_out, graphs_out, docs_out, prompt, pattern, max_count, skip_graphs, skip_llm, fail_fast, dot_path, no_project_enrich, include_archives, debug_discovery):
    ...
    try:
        if not skip_llm and os.environ.get("OPENAI_API_KEY"):
            ...
            header, body = _parse_plan_front_matter(raw)

            # determine needed executions based on docs-per-round
            docs_spec = header.get("docs") or []
            docs_per_round = int(os.environ.get("DOCS_PER_ROUND", "3"))
            pending = [d for d in docs_spec if d and not d.get("status") == "done"]
            needed = max(1, (len(pending) + docs_per_round - 1) // docs_per_round)

            meta = header.setdefault("meta", {})
            # If not present or smaller than needed, set it (first run)
            ...
            if not runner.exists():
                ...
                while True:
                    ...
                    remaining = [d for d in docs_spec if d and d.get("status") != "done" and not (docs_out / d.get("filename","")).exists()]
    ...
    # === After docs are generated, repair/ensure MKDocs ===
    try:
