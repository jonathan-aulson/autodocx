...
def run_scan(repo: Path, out: Path, debug: bool = False, mkdocs_build: bool = False, llm_rollup: bool = False):
    ...
    for idx, sig in enumerate(signals):
        try:
            ...
            sir_obj = {
                ...
                "name": name,
                "file": props.get("file", ""),
                "component_or_service": props.get("component_or_service") or props.get("service") or "",
                "props": props,
    ...
    (out_base / "artifacts.json").write_text(json.dumps(artifacts, indent=2), encoding="utf-8")
    if debug:
    ...
    try:
        groups = group_by_component(out_base)
        if debug:
