...
- `interfaces`: list exposed entry points (REST/SOAP/queue/timer/CLI) based on triggers, routes, process_flows, palette roles, and relationship data. Include endpoint/method information and evidence_ids; for BW, surface partnerLinks and mapper IO.
...
- `interdependencies`: capture “calls”, “called_by”, and “shared_data” relationships from `interdependencies_slice` plus repository-level data. Highlight shared datastores/identifiers and upstream/downstream calls; omit this section if no evidence.
- `packaging` and `artifacts`: surface bundle/module/docker identifiers, manifests, and related artifacts (Dockerfile, MANIFEST, configs) with evidence_ids.
...
produce JSON with the exact sections required by the schema. Follow these rules:
 ...
 - `interdependencies`: turn `interdependencies_slice` (calls/called_by/shared_data) into an “Interdependency map” that clearly lists process calls, external services, and shared datastores; cite evidence_ids (or leave empty if uncertain).
 - `key_inputs` / `key_outputs`: rely on payload_examples, inputs/outputs examples, schemas, and experience packs; avoid placeholders.
 ...
 - `packaging` / `artifacts`: include bundle/module/docker identifiers plus key artifacts (Dockerfile, MANIFEST, configs) with evidence_ids when present.
